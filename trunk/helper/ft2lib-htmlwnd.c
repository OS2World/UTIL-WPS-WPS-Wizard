...
LONG gpiStringLength(HPS hps, LONG lLength, char * chrText)
{
  ...
  Ft2QueryTextBox(hps, lLength, chrText, TXTBOX_COUNT, aptl);
...
BOOL isBitmapFont(HPS hps, char * chrFacename)
{
  ...
  cFonts=Ft2QueryFonts(hps, QF_PUBLIC, chrName, &lTemp, sizeof(FONTMETRICS), NULL);
  ...
  if((pfm=(PFONTMETRICS)malloc(cFonts*sizeof(FONTMETRICS)))!=NULLHANDLE)
    {
      ...
      lTemp=cFonts;
      cFonts=Ft2QueryFonts(hps, QF_PUBLIC, chrName, &cFonts, sizeof(FONTMETRICS), (PFONTMETRICS)pfm);
...
void setHeadingFormat(HPS hps, LONG * clrFore)
{
  ...
  Ft2CreateLogFont(hps, NULL, 1L, &fat);
  Ft2SetCharSet(hps, 1L);

  //sizef.cx=MAKEFIXED(18,0);
  //sizef.cy=MAKEFIXED(18,0);
  // Ft2QueryCharBox(hps, &sizef);
...
void setFattr(HPS hps, char * chrFacename, ULONG ulPts, FATTRS* fat)
{
  ...
  /* Get num fonts of this font family */
  cFonts=Ft2QueryFonts(hps, QF_PUBLIC, chrFacename, &lTemp, sizeof(FONTMETRICS), NULL);
  ...
  if((pfm=(PFONTMETRICS)malloc(cFonts*sizeof(FONTMETRICS)))!=NULLHANDLE)
    {
      ...
      lTemp=cFonts;
      cFonts=Ft2QueryFonts(hps, QF_PUBLIC, chrFacename, &cFonts, sizeof(FONTMETRICS), (PFONTMETRICS)pfm);
...
void queryFonts(HPS hps)
{
  ...
  /* Get num fonts */
  cFonts=Ft2QueryFonts(hps, QF_PUBLIC, NULLHANDLE /*theFont*/, &lTemp, sizeof(FONTMETRICS), NULL);
  ...
  if((pfm=(PFONTMETRICS)malloc(cFonts*sizeof(FONTMETRICS)))!=NULLHANDLE)
    {
      ...
      lTemp=cFonts;
      cFonts=Ft2QueryFonts(hps, QF_PUBLIC, NULL /*theFont*/, &cFonts, sizeof(FONTMETRICS), (PFONTMETRICS)pfm);
...
void selectHeadingFont(HPS hps, FATTRS *fat, char * fontName)
{
  ...
  if(!isBitmapFont(hps, fontName)) {
  ...
  else
    {
      ...
      /* Get num fonts of this font family */
      cFonts=Ft2QueryFonts(hps, QF_PUBLIC, "Helv", &lTemp, sizeof(FONTMETRICS), NULL);
      ...
      if((pfm=(PFONTMETRICS)malloc(cFonts*sizeof(FONTMETRICS)))!=NULLHANDLE)
        {
          ...
          lTemp=cFonts;
          cFonts=Ft2QueryFonts(hps, QF_PUBLIC, "Helv", &cFonts, sizeof(FONTMETRICS), (PFONTMETRICS)pfm);
...
void setHeadingFormat(HPS hps, LONG * clrFore, HWND hwnd)
{
  ...
  if(wndData->fatHeading.lMaxBaselineExt!=0) {
    /* It's a bitmap font */
    ret=Ft2CreateLogFont(hps, NULL, 1L, &wndData->fatHeading);
    Ft2SetCharSet(hps, 1L);
  }
  else {
    SIZEF sizef;
...
void setLinkFormat(HPS hps, LONG * clrFore, HWND hwnd)
{
  ...
  ret=Ft2CreateLogFont(hps, NULL, 1L, &fat);
  Ft2SetCharSet(hps, 1L);
...
void setDefaultFormat(HPS hps, LONG * clrFore, HWND hwnd)
{
  ...
  Ft2SetCharBox(hps, &wndData->sizef);
  ret=Ft2CreateLogFont(hps, NULL, 1L, &wndData->fatText);
  Ft2SetCharSet(hps, 1L);
}
...
LONG doFormat(HPS hps, HWND hwnd, LONG lLength, char * chrText, RECTL* rcl,   TAGINFO *tagInfo, LONG lCharsDone,  HTMLWNDDATA* wndData)
{
  ...
  Ft2QueryTextBox(hps, 1, "*", TXTBOX_COUNT, aptl); /* Get size of a character. Not accurate but sufficient. */

  Ft2QueryCurrentPosition(hps, &ptl); /* Current position in the hps */
  ptlMove=ptl;
    ...
    case MARK_H1:
      {
        ...
        Ft2QueryTextBox(hps, 1, "*", TXTBOX_COUNT, aptl);
    ...
    case MARK_H1_END:
      {
        ...
        Ft2SetCharSet(hps, LCID_DEFAULT); /* Default font  */
        Ft2DeleteSetId(hps, LCID_ALL);
  ...
  if( lPtl <= (rcl->xRight-ptl.x)) {
  ...
  else
    {
      ...
      while((lLength=newStringLength( hps, lTotalLength-lDone, chrText)) && lDone+lLength<=lTotalLength)
        {
          lPtl=gpiStringLength(hps, lLength, chrText);
          Ft2QueryCurrentPosition(hps, &ptl);
          ptlMove=ptl;
          ...
          if(lPtl > (rcl->xRight-ptl.x))
            {
              ...
              if(lPtl <= (rcl->xRight-fi.xPos))
                ...
                if(lLength) {
                  //Ft2CharString(hps, lLength, chrText);
                  lPtl=gpiStringLength(hps, lLength, chrText);
                  ptlMove.x+=lPtl;
                  Ft2Move(hps, &ptlMove);
                }
...
static void formatText(HWND hwnd, RECTL *rcl)
{
  ...
  Ft2QueryCharBox(hps, &wndData->sizef);
  ...
  while(hTemp)
    {
      switch(hTemp->iType)
        ...
        case MARK_LINK_END:
          {
            Ft2SetCharSet(hps, LCID_DEFAULT);
            Ft2DeleteSetId(hps, LCID_ALL);
            //            Ft2SetColor(hps, CLR_GREEN /*curColor*/);
               clrFore=curColor;
            break;
          }
        case MARK_UNDERLINED:
      ...
      Ft2Move(hps, &ptl);
      Ft2CharString(hps, hTemp->iLength, hTemp->chrText);
